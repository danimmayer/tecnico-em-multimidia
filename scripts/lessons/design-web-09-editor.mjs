...
export const tabletStart = shrink(desktopStart, 'tablet');
export const phoneStart = shrink(desktopStart, 'phone');
...
export function checks(fmt, parts, reference = parts) {
  ...
  const cards = items.filter(p => p.kind === 'card'), expected = reference.filter(p => p.kind === 'card' && !p.decorative).length;
  if (fmt !== 'desktop') list.push({ok: cards.length === expected, text: cards.length === expected ? `Os ${expected} campeonatos continuam na tela` : `Faltam campeonatos: o computador tem ${expected}`});
  ...
  if (fmt === 'phone') {
    ...
    const menu = !!header && textWidth(header.sub ?? 'Campeonatos · Horários · Contato', 16, header.font || 'Arial, sans-serif') <= header.w * .46;
...
export function illustrations() {
  return {
    ...
    'so-encolher': renderTrio([['phone', phoneStart, 'Só encolhida'], ['phone', phoneExample, 'Organizada para o celular']], true).replace('A página da Arena Pixel no computador, no tablet e no celular', 'A página do computador apenas encolhida no celular, ao lado da versão organizada para o celular'),
...
function startEditor() {
  ...
  const tips = {
    ...
    tablet: 'Tablet: grade de 8 colunas. Os cartões precisam de outra organização.',
    phone: 'Celular: grade de 4 colunas. Tudo empilhado; a linha tracejada marca o fim da primeira tela.'
  };
  let state = {desktop: copy(desktopStart), tablet: copy(tabletStart), phone: copy(phoneStart), background: {desktop: '#F4F1FA', tablet: '#F4F1FA', phone: '#F4F1FA'}}, fmt = 'tablet', selected = {desktop: 'header', tablet: 'header', phone: 'header'};
  ...
  function draw() {
    ...
    $('fromdesktop').hidden = fmt === 'desktop';
  ...
  $('fromdesktop').addEventListener('click', () => {
    if (!confirm(`Recomeçar a tela ${formats[fmt].label} a partir do computador? Dá para desfazer.`)) return;
    remember(); state[fmt] = shrink(state.desktop, fmt); selected[fmt] = state[fmt][0].id; draw();
    announce(`Tela ${formats[fmt].label} recomeçada: a página do computador apenas encolhida. Organize na grade de ${formats[fmt].cols} colunas.`);
  });
  ...
  $('example').addEventListener('click', () => showCompare('Uma organização possível', [['tablet', tabletExample], ['phone', phoneExample]], 'No tablet, cartões dois a dois. No celular, tudo empilhado, com destaque e botão na primeira tela e o menu encurtado. Outras organizações que passem na conferência também valem.'));
  ...
  try {
    const request = indexedDB.open('oficina-responsiva-v1', 1);
