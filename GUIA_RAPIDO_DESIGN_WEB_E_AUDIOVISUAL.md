# Caderno de preparação das aulas

**Design Web e Produção Audiovisual · Prof. Daniel Marcos Mayer · 2026**

## Princípio de trabalho

Organizei cada encontro em quatro movimentos: conceito, demonstração, atividade e
...
Antes da entrada da turma:

1. planejamento da aula aberto no navegador;
2. recursos, arquivos e equipamentos separados;
3. demonstração executada uma vez;
4. entrega e critérios conferidos;
5. alternativa definida para falta de internet, software ou equipamento.

As anotações ficam ocultas por padrão. A tecla **P** alterna entre a projeção da
turma e minhas notas de condução.

## Ritmo de trabalho com adolescentes

O ritmo que adoto evita explicações longas:

1. apresentação do foco em até 10 minutos;
2. demonstração curta, com o resultado visível;
3. prática em duplas ou equipes com papéis definidos;
4. parada para conferência ainda durante o bloco;
5. registro ou arquivo salvo antes da mudança de atividade;
6. fechamento cinco minutos antes do término.

Quem ainda não conseguiu iniciar recebe prioridade. Acabamento e efeitos ficam
para depois da entrega mínima funcionar.
...
As análises usam telas preparadas e dados fictícios; aplicativos bancários,
contas e mensagens pessoais permanecem fechados. A acessibilidade é estudada
como auditoria de barreiras com teclado, zoom, foco e leitor de tela, sem
simulação de deficiência.

### Aulas 23 a 29 · Métricas e projeto integrador

Métricas são trabalhadas com relatório fictício ou conta institucional
autorizada. O projeto final mantém escopo de uma página inicial, uma página
interna e uma tarefa principal. A defesa prevista no planejamento dura sete
minutos.
...
O destino de cada gravação é registrado separadamente:

- exercício sem exibição;
- exibição interna à turma;
- mostra com convidados;
- publicação online.

Uso de imagem depende do modelo institucional aprovado e do escopo autorizado.
Para menores, a escola define a participação do responsável legal. Sem
autorização suficiente, a gravação usa objetos, espaços, mãos não identificáveis
ou permanece restrita ao uso permitido.

### Captação

Antes de cada tomada: lente, estabilidade, foco, exposição, enquadramento e som.
...
alternativa baseada nos mesmos conceitos de importar, selecionar, cortar,
mixar e exportar; os menus não são tratados como equivalentes.

### Exportação

O padrão de referência do projeto final é:

- contêiner MP4;
- codec H.264;
- resolução 1920 × 1080;
- taxa de quadros igual à captação;
- áudio AAC, 48 kHz;
- nome `turma_grupo_projeto_v01.mp4`.

Versões verticais só são exigidas quando o enquadramento 9:16 tiver sido previsto
na captação.

### Mostra

Dois blocos ficam reservados para as exibições. A ordem, o tempo de apresentação,
a duração do vídeo e a devolutiva são conferidos antes da aula. Arquivos locais e
cópias de segurança substituem qualquer dependência de streaming.
...
- equipamento indisponível → mesma intenção de linguagem com smartphone.
